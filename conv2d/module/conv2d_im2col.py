...
class Conv2D:
    ...
    def forward(self, input_tensor):
        ...
        print(f"[DEBUG] input_col shape: {input_col.shape}, kernels_col.T shape: {kernels_col.T.shape}")  # Kiểm tra kích thước phép nhân ma trận

        output_col = input_col @ kernels_col.T  
        output = output_col.reshape(batch_size, output_height, output_width, self.out_channels)
        output = output.transpose(0, 3, 1, 2)

        if np.isnan(output).any() or np.max(output) > 1000:
            print(f"[ERROR] Output bad! max(output) = {np.max(output)}")

        output_tensor = torch.tensor(output, dtype=torch.float32)
        print(f"[FORWARD] Output shape: {output_tensor.shape}")
    ...
    def backward(self, d_output):
        ...
        print(f"[DEBUG] d_output_col shape: {d_output_col.shape}")  # Kiểm tra kích thước của gradient

        d_kernels = d_output_col.T @ input_col  
        d_kernels = d_kernels.reshape(out_channels, in_channels, kernel_size, kernel_size)

        if np.isnan(d_kernels).any():
            print("[ERROR] NaN detected in d_kernels!")
        if np.max(d_kernels) > 1000:
            print(f"[WARNING] Gradient over max: max(d_kernels) = {np.max(d_kernels)}")
